...
impl RateLimiter {
    ...
    pub fn check(&self, node_id: &[u8; 32]) -> bool {
        ...
        if elapsed >= Duration::from_secs(1) {
            ...
            *tokens =
                (self.requests_per_second as f32 * periods).min(self.requests_per_second as f32);
            *last_reset = now;
        }
    ...
    pub fn get_bucket_info(&self, node_id: &[u8; 32]) -> Option<(Duration, f32)> {
        self.buckets
            ...
            .unwrap()
            .get(node_id)
            .map(|(instant, tokens)| (instant.elapsed(), *tokens))
    }
}
...
impl SlidingWindowRateLimiter {
    ...
    pub fn check(&self, node_id: &[u8; 32]) -> bool {
        ...
        let window = windows.entry(*node_id).or_default();
...
mod tests {
    ...
    fn test_token_bucket_rate_limiting() {
        ...
        assert!(
            ...
            "La 6ª solicitud debe ser rechazada"
        );
    ...
    fn test_sliding_window_rate_limiting() {
        ...
        assert!(
            !limiter.check(&node_id),
            "La 4ª solicitud debe ser rechazada"
        );
        ...
        assert!(
            limiter.check(&node_id),
            "Debe permitir después de nueva ventana"
        );
    }
}
